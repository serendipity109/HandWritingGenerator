...
# 設定gen_images_num（生成數量），semantic_source（語料庫），char_img_list_path（相對應的json檔），
# images_dst_folder（輸出的資料夾），然後run。

gen_images_num = 100
# gen_images_num = 300000

wd = os.getcwd()
semantic_source = wd + '/utils/char_source/300.txt'
char_img_list_path = wd + '/utils/chnumengds.json'
images_dst_folder = './tmp'
tmp_text_path = wd + "/tmp_text.txt"

if not os.path.exists(images_dst_folder):
    os.makedirs(images_dst_folder)
...
def add_bg(img):
    bg_lst = glob.glob('/Users/wangshunda/general-python-projects/Tools/text_renderer-master/example_data/bg/*')
...
def generate_img(img, q=None):
    ...
    # text_img.save(images_dst_folder + "/300" + filename + '_2.jpg')
    dst_img = ea.do_variation(text_img, filename)
    dst_img.save(images_dst_folder + "/300" + filename + '.jpg')
    text = u''.join(selected_char_list)
    line = "300" + filename + '.jpg ' + text
